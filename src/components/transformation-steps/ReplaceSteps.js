...
export function ReplaceStep({ type, setTransformer, disabled, setTitle, setHelp }) {
  ...
  useEffect(() => setTitle(type), [setTitle, type]);
  ...
  return (
    <>
      <TextField
        ...
        type="text"
        label={type === STRING_REPLACE_TYPE ? "From" : "Regex"}
...
export function SubstringStep({ setTransformer, disabled, setTitle, setHelp }) {
  ...
  useEffect(() => setTitle("Substring"), [setTitle]);
  ...
  }, [fromValue, toValue, setTransformer, setHelp]);
...
export function InsertStep({ setTransformer, disabled, setTitle, setHelp }) {
  ...
  useEffect(() => setTitle("Insert at"), [setTitle]);
  ...
  useEffect(() => {
    setHelp(
      <>
        ...
        <div>
          Input <code>"abc"</code>, index: 1, text: '!'
        </div>
        <div>
          Output will be <code>a!bc</code>
        </div>
        <div>
          Input <code>"abc"</code>, index: -1, text: '!'
        </div>
        <div>
          Output will be <code>ab!c</code>
        </div>
...
export function AppendStep({ setTransformer, disabled, setTitle, setHelp }) {
  const [fromValue, setFromValue] = useState("");
  useEffect(() => setTitle("Append"), [setTitle]);
  ...
  }, [fromValue, setTransformer, setHelp]);
